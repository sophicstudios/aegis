...
Describe d("agtm_vector2", []
{
    ...
    it("Multiplication", [&]
    {
        ...
        expect(verify(v1, 2, 6)).toBeTrue();
        
        Vector2<int> v3;
        v3 = v1 * v2;

        expect(verify(v3, 4, 18)).toBeTrue();
        
        v3 *= 2;

        expect(verify(v3, 8, 36)).toBeTrue();
        
        Vector2<int> v4;
        v4 = v3 * 2;

        expect(verify(v4, 16, 72)).toBeTrue();
    });

    it("Division", [&]
    {
        Vector2<int> v1(16, 72);
        Vector2<int> v2(2, 3);
        
        v1 /= v2;

        expect(verify(v1, 8, 24)).toBeTrue();
        
        Vector2<int> v3;
        v3 = v1 / v2;

        expect(verify(v3, 4, 8)).toBeTrue();
        
        v3 /= 2;

        expect(verify(v3, 2, 4)).toBeTrue();
